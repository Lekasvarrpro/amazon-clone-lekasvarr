...
const Sidebar = () => {
  ...
  const toggleSearch = () => setShowSearch((prev) => !prev); // <-- TOGGLE
  ...
  return (
    <>
      <div className="flex items-center gap-5 text-white pr-10">
        ...
        {/* Search Button */}
        <button
          onClick={toggleSearch}   // <-- TOGGLE POPUP
        ...
        <div className="relative group">
          ...
          <div className="absolute leading-3 right-0 mt-2 w-100 max-h-64 overflow-y-auto bg-black/50 backdrop-blur-lg border border-white/30 rounded-md grid grid-cols-5 gap-2 opacity-0 invisible group-hover:visible group-hover:opacity-100 transition-all z-50 p-2 shadow-md">
            {languages.map((lang) => (
              <button
                key={lang.code}
        ...
        <div className="relative group">
          ...
          <div className="absolute right-0 mt-2 w-100 bg-black/50 backdrop-blur-lg border border-white/30 rounded-md font-semibold grid grid-cols-2 gap-2 opacity-0 invisible group-hover:visible group-hover:opacity-100 transition-all z-50 p-3 shadow-md">
        ...
        <div className="relative group">
          ...
          <div className="absolute right-0 mt-2 bg-black/50 backdrop-blur-lg border border-white/30 rounded-xl p-4 shadow-xl grid grid-cols-1 gap-2 w-44 opacity-0 invisible group-hover:visible group-hover:opacity-100 transition-all duration-200">
            <p className="text-gray-300 text-md text-center">Your account</p>

            <button className="text-white font-semibold hover:bg-white hover:text-black rounded-lg p-2 cursor-pointer text-sm text-left">
              Sign In
            </button>

            <button className="text-white font-semibold text-sm text-left hover:bg-white cursor-pointer hover:text-black rounded-lg p-2">
              Help
            </button>

            <button className="text-white font-semibold text-sm text-left hover:bg-white cursor-pointer hover:text-black p-2 rounded-lg">
              Watch Anywhere
            </button>
          </div>
        </div>
        ...
        <div className="cursor-pointer">
          <NavLink
            to="/join"
            className="cursor-pointer font-semibold bg-sky-600 hover:bg-sky-400 hover:shadow-md p-2.5 pl-5 pr-5 rounded-lg transition"
          >
            Join Prime
          </NavLink>
        </div>
      ...
      {/* POPUP Search Box */}
      {showSearch && <SearchBox onClose={toggleSearch} />}
    </>
  );
};
